            ...
            steps {
                sshagent (credentials: [env.SSH_CREDENTIALS]) {
                    script {
                        ...
                        // Write service file without triple quotes (avoids 0xFFFF bugs)
                        sh """
                        ssh ${EC2_USER}@${EC2_HOST} 'sudo bash -c "cat > /etc/systemd/system/ecommerce.service <<EOF
[Unit]
...
EOF"'
                        ...
                        'sudo systemctl daemon-reload && sudo systemctl restart ecommerce.service'
                        """
                    }
                }
            }
